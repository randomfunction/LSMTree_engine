...
size_t BloomFilter::HashDJB2(const string& key) {
    size_t hash = 5381;
...
size_t BloomFilter::HashFNV1a(const string& key) {
...
size_t BloomFilter::HashRotate(const string& key) {
    size_t hash = 0;
    for (string::const_iterator it = key.begin(); it != key.end(); it++) {
        unsigned char ch = static_cast<unsigned char>(*it);
...
// necesary for store bloomfilter data in sstable to disk 
vector<uint8_t> BloomFilter::Serialize() const {
    ...
    TraceLogger::Log("BLOOM","Serialized Bloom filter bytes="+to_string(bytes.size())+"bit_count="+to_string(bit_count_));
    return bytes;
}

BloomFilter BloomFilter::Deserialize(size_t bit_count, const vector<uint8_t>& bytes) {
